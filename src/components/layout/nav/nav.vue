    ...
    <ul class="nav-ul">
      <template v-for="(item, index) in navList" :key="index">
        ...
        >
          ...
          <span class="iconfont" v-if="item.children">{{
            item.moreTag ? '&#xe835;' : '&#xe833;'
          }}</span>
        </li>
        <ul
          class="nav-ul-child"
          :class="{ 'show-more': item.moreTag }"
          v-if="item.children"
          :key="index + 200"
        >
          <li
            ...
            :key="num + 100"
            :class="{ 'active-route': iscurrentRoute(child) }"
            @click="_hanleLi(child)"
          >
    ...
    const iscurrentRoute = (item: any): string => {
      ...
      if (children) {
        ...
        for (const child of children) {
          if (
            child.path === curPath ||
            (child.matchKey && curPath.includes(child.matchKey))
          ) {
            return 'active-route-parent'
          }
        }
      } else {
        return path === curPath || (matchKey && curPath.includes(matchKey))
          ? 'active-route'
          : ''
      }
      return ''
